...
set.seed(4)
data = generateCovariance(8, 8)

k = 0
phi = 1

# Compute weight matrix
W1 = cggmWeights(data$sample, phi = phi, k = k)
...
# Compute weight matrix
W2 = cggm_weights(data$sample, phi = phi, k = k, connected = TRUE)

W1 - W2

# Plot the weight matrix as a weighted graph to check whether we see the same
# number of connected components
G = graph_from_adjacency_matrix(W1, mode = "undirected", weighted = TRUE)
plot(G, edge.label = round(E(G)$weight, 3), layout = layout.circle(G))
# Plot the weight matrix as a weighted graph to check whether we see the same
# number of connected components
G = graph_from_adjacency_matrix(W2, mode = "undirected", weighted = TRUE)
plot(G, edge.label = round(E(G)$weight, 3), layout = layout.circle(G))
