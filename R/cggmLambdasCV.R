...
#' @param knn The number of nearest neighbors that should be used to set weights
...
#'
#' @export
cggmLambdasCV <- function(X, lambdas, phi, knn, kfold, cov_method = "pearson",
                          gss_tol = 1e-4, conv_tol = 1e-9,
                          ...
                          max_iter = 2000, use_Newton = TRUE)
{
    ...
    for (fi in 1:kfold) {
        ...
        W.train = cggmWeights(S.train, phi = phi, k = knn)
